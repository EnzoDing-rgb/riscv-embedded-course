...
│  QEMU 仿真 · GDB 调试              ringbuf · shell · 数据流水线  环境哨兵 · 量化对比表    │
└────────────────────────────────────┴────────────────────────────────────────────────┘
...
**硬件（全班统一）**：RISC-V 开发板（推荐 [RevyOS 支持硬件](https://github.com/ruyisdk/support-matrix) 中的 Lichee Pi 4A / Milk-V Meles / 同档次板卡）+ I2C 温湿度（如 SHT30/AHT20）+ 1 颗 LED（异常指示）。无物理板可用 QEMU 仿真。
...
| **RevyOS** | W3–W4, W7–W8 | 运行在 RISC-V 板上的 Debian 发行版，提供完整 Linux 用户空间 |
| **QEMU (via ruyi)** | W1–W2, W7 | 无板仿真：`ruyi install emulator/qemu` |
...
| **RISC-V Linux 板**（RevyOS 或发行版） | **W3–W4, W7–W8** | C 固件/用户态程序：传感器驱动、ringbuf、shell；**int8 推理**；GPIO/I2C 真外设 | ❌ 不安装 PyTorch；❌ 不在板上做反向传播训练 |
...
```text
  RISC-V 板 W4 采集 ──导出 CSV/日志──→  Linux PC W5 训练 + 量化
                                               │
                                               ↓ weights.h
  RISC-V 板 W7–W8 ←──scp/烧录含权重的程序─────  PC 上 ref_inference.c 对 golden
```
...
**前提**：C 语言 + 计算机组成原理  
**硬件**：RISC-V 开发板（班内统一型号）；无板可用 QEMU  
...
| 问题 | 本课怎么答 |
|------|------------|
| 学嵌入式能干嘛？ | 写 **传感采集与推理固件**：驱动、缓冲、数据处理流水线，为 AIoT 量产打基础 |
...
每周：理论 ~4h，Lab ~8h，复盘 ~3h。W1–W2 可用 QEMU；W3 起重真板；W5–W6 PC 对 golden 后再上板。
...
- **RuyiSDK 全景**：包管理器 (`ruyi`) + IDE + RevyOS + 社区生态  
...
**硬件环境**：RISC-V 开发板运行 RevyOS（或等效 Debian 发行版），通过 RuyiSDK 工具链交叉编译用户态程序部署到板上。
...
在 RISC-V Linux 板上开发外设驱动，比裸机 MMIO 更接近 AIoT 产品实际开发模式——用标准 Linux 接口，可移植、可调试、可复用。
...
**本段目标**：在 **第 1–4 周数据采集系统** 上部署 **自写 int8 推理**，完成 AIoT 闭环。  
...
| 推理 | PC 上 `ref_inference.c` 作 golden（int8 参考实现）；用 RuyiSDK 工具链编译验证 |
...
> **运行环境：量化与对比表在 Linux PC 完成**；**自写 C 算子**可在 PC 先通过 golden 测试，W7 再链入 RISC-V 工程。
...
| 书名 | 作者 | 出版年 | 说明 |
|------|------|--------|------|
| **《计算机组成与设计：硬件/软件接口 (RISC-V版)》** 原书第2版 | David A. Patterson, John L. Hennessy | 2020 | **首选教材**。两位图灵奖得主撰写，RISC-V 领域"圣经"。配套中国大学 MOOC 课程。英文版 *Computer Organization and Design: The Hardware/Software Interface, RISC-V Edition* |
...
- 无板学生：W1–W2 用 QEMU 用户模式；W3 起建议至少 2 人一块板；W7–W8 可用 QEMU 系统模式仿真推理（精度一致，延迟不同，答辩时注明）
...
- GPIO：手册寄存器点灯（MMIO），不用厂商 HAL 黑盒  
- OpenOCD 烧录与 GDB 调试

### Lab

1. 裸机 Makefile + 链接脚本，生成 `.elf`，`make flash` 烧录  
2. 真板点灯（MMIO GPIO）；画启动流程图 + 内存段地址表  
3. 交付：工程 + 点灯视频 + 内存段标注
...
截至 2025 年 12 月，RevyOS 已支持 10+ 款 RISC-V 硬件平台，包括：
...
镜像下载：[ISCAS 开源镜像站](https://mirror.iscas.ac.cn/ruyisdk/)
